...
const PostFooter = ({ subject, currentPost, postList }: PostFooterProps) => {
  const subjectColor = getSubjectColor(subject);
  // 현재 포스트의 인덱스 찾기
  const currentIndex = postList.findIndex((p) => p.post === currentPost);
    ...
    >
      {prevPost ? (
        <Link
          ...
          className={`group relative flex flex-col gap-1.5 overflow-hidden rounded-lg border border-gray-200 bg-gradient-to-br from-gray-50
            to-white p-3 shadow-sm transition-all duration-300 hover:border-gray-300 hover:shadow-md dark:border-gray-700
            dark:from-gray-800 dark:to-gray-900 dark:hover:border-gray-600 sm:gap-2 sm:rounded-xl sm:p-5 ${
      ...
      {nextPost ? (
        <Link
          href={nextPost.url}
          className={`group relative flex flex-col gap-1.5 overflow-hidden rounded-lg border border-gray-200 bg-gradient-to-br from-gray-50
            to-white p-3 text-right shadow-sm transition-all duration-300 hover:border-gray-300 hover:shadow-md dark:border-gray-700
            dark:from-gray-800 dark:to-gray-900 dark:hover:border-gray-600 sm:gap-2 sm:rounded-xl sm:p-5
            ${prevPost ? "flex-1 sm:ml-4" : "w-full"}`}
        >
          <div className="flex items-center justify-between gap-2 text-xs text-gray-500 dark:text-gray-400">
            <div className="flex items-center gap-1 sm:gap-1.5">
              <IconCalendar className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
